...
///
/// - **ストレージ**
///   - `Ballot` と `Log` の二つの構造体を永続化するために用いる
///      - `Ballot` は、論文における currentTerm と votedFor の二つを表現する
///          - 二つをまとめて表現して問題はないか？
///          - 未voteだが、currentTermだけが存在するようなケースは存在するか？
///      - `Log` は、論文における `log[]` を表現する
